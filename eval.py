# adapted from https://github.com/kenchan0226/keyphrase-generation-rl/blob/master/evaluate_prediction.py and https://github.com/xiaowu0162/low-resource-kpgen/blob/main/utils/evaluate.py
import os
import re
import sys
import json
...
def process_output(str_list):
    phrases = str_list
    phrases = [re.sub(r'\(.*?\)|\[.*?\]|\{.*?\}', '', kw) for kw in phrases]
    phrases_token = [meng17_tokenize(kw) for kw in phrases]
    #phrases_token = [replace_numbers_to_DIGIT(kw, k=2) for kw in phrases_token]
    phrases_str = [ utils.listToStr(p) for p in phrases_token]
    phrases_str = [ p.lower() for p in phrases_str]
    phrases_seq_str = ';'.join(phrases_str)
    phrases_seq_str = phrases_seq_str.strip()
    return phrases_seq_str
...
def main(predictions, exp_path, result_file_suffix, k_list=[5, 'M']):
    ...
    print(f'{result_file_suffix}')
    print(result_txt_str, end='\n\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred_dir_path', type=str, required=True, help="Directory path of the prediction files")
    parser.add_argument('--k_list', nargs='+', default=[3, 5, 10, 'M'], help='K values for evaluation')
    parser.add_argument('--dataset_names', nargs='+', default=['inspec', 'krapivin', 'nus', 'semeval', 'kp20k'], help='List of testset')

    args = parser.parse_args()


    dir_path = args.pred_dir_path
    ...
    for file_name in files:
        ...
        outputs = utils.read_json_by_line(file_path)

        pred_keys_list = []
        for d in outputs:
            generated_output = d['generated_ourput']
            generated_keyphrases = utils.extract_keyphrases_from_output(generated_output)
            pred_keys_seq = process_output(generated_keyphrases)
            pred_keys_list.append(pred_keys_seq)
        
        utils.save_list_to_txt(pred_keys_list, file_path.replace('.json', '_pred.txt'))



    for dataset_name in datasets:

        srcs_path = f'data/{dataset_name}/test_src.txt'
        tgts_path = f'data/{dataset_name}/test_trg.txt'

        src_list = read_txt_to_list(srcs_path)
        src_list = [src.lower() for src in src_list]

        tgts = read_txt_to_list(tgts_path)
        tgts = [tgt.lower() for tgt in tgts]

        files = os.listdir(dir_path)
        pred_files = [file for file in files if file.startswith(dataset_name) and file.endswith('pred.txt')]
        pred_file = pred_files[0]
        pred_file_path = dir_path + '/' + pred_file

        preds = read_txt_to_list(pred_file_path)
        ...
        for pred, tgt in zip(preds, tgts):
            ...
            for t in t_list:
                t = t.strip()
                t = t.replace('< digit >', '<digit>')
                mod_t_list.append(t)

            pred_list.append(mod_p_list)
            tgt_list.append(mod_t_list)
            
        log = pred_file

        run_eval((pred_list, tgt_list, src_list), dir_path, log, k_list)
